(() => {
  // frappe-html:/home/gauritrans/frappe-bench/apps/hrms/hrms/public/js/templates/employees_with_unmarked_attendance.html
...
{% } %}`;
  ...
  // frappe-html:/home/gauritrans/frappe-bench/apps/hrms/hrms/public/js/templates/feedback_summary.html
...
`;
  ...
  // frappe-html:/home/gauritrans/frappe-bench/apps/hrms/hrms/public/js/templates/feedback_history.html
  ...
  // frappe-html:/home/gauritrans/frappe-bench/apps/hrms/hrms/public/js/templates/rating.html
...
`;
  ...
  $.extend(hrms, {
    ...
    handle_realtime_bulk_action_notification: (frm, event, doctype) => {
      ...
      frappe.realtime.on(event, (message) => {
        hrms.notify_bulk_action_status(doctype, message.failure, message.success, message.for_processing);
        if (message.success)
          frm.refresh();
      });
    },
    notify_bulk_action_status: (doctype, failure, success, for_processing = false) => {
      ...
      if (failure.length) {
        ...
        message += __("Check <a href='/app/List/Error Log?reference_doctype={0}'>{1}</a> for more details", [doctype, __("Error Log")]);
        title = __("Failure");
        indicator = "red";
        if (success.length) {
          message += "<hr>";
      ...
      if (success.length) {
        ...
        message += __("<table class='table table-bordered'><tr><th>{0}</th><th>{1}</th></tr>", [__("Employee"), doctype]);
  ...
  hrms.payroll_utils = {
    set_autocompletions_for_condition_and_formula: function(frm, child_row = "") {
      ...
      frappe.run_serially([
        ...["Employee", "Salary Structure", "Salary Structure Assignment", "Salary Slip"].map((doctype) => frappe.model.with_doctype(doctype, () => {
          autocompletions.push(...frappe.get_meta(doctype).fields.map((f) => ({
            value: f.fieldname,
            score: 8,
        ...
        () => {
          frappe.db.get_list("Salary Component", {
            ...
            autocompletions.push(...salary_components.map((d2) => ({
              value: d2.salary_component_abbr,
              score: 9,
              meta: __("Salary Component")
            })));
            autocompletions.push(...["base", "variable"].map((d2) => ({
              value: d2,
              score: 10,
            ...
            if (child_row) {
              ["condition", "formula"].forEach((field) => {
                frm.set_df_property(child_row.parentfield, "autocompletions", autocompletions, frm.doc.name, field, child_row.name);
              });
...
//# sourceMappingURL=hrms.bundle.BICFAENM.js.map
